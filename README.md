...
The ICE/TURN server list lives in ```./server/src/ice-server-array.ts``` and is exported as the ```IceServers``` array. Replace the entries there with your own (this should work for paid plans on [Metered](https://www.metered.ca/), for other TURN servers you may need to edit the source code).

If you don't need the reliability of a TURN server, you can just copy a free STUN server array e.g. [here](https://gist.github.com/mondain/b0ec1cf5f60ae726202e) - choose less than 6 and put them into ```./server/src/ice-server-array.ts```, you can remove the username and credential attributes.
...
Parsing of the messages is done in ```./frontend/src/position/client-positions.ts```.
